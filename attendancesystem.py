...
def manually_fill():
    ...
    sb.geometry('580x320')
    sb.configure(background='bg.jpg')
    ...
    def fill_attendance():
        ...
        global subb
        subb=SUB_ENTRY.get()
        DB_table_name = str(subb + "_" + Date + "_Tim_" + Hour + "_" + Minute + "_" + Second)
        ...
        try:
            ...
            while (True):
                ...
                for (x, y, w, h) in faces:
                    ...
                    if os.path.isdir("TrainingImage"):
            ...
            csvFile = open("StudentDetails.csv", mode="a+")
            writer = csv.writer(csvFile, delimiter=',')
            writer.writerow(row)
            csvFile.close()
            df1 = pd.read_csv("StudentDetails.csv")
            df1.columns=["Enrollment", "Name", "Date", "Time"]
            df1 = df1.append({'Enrollment': Enrollment, 'Name': Name, 'Date': Date,"Time":Time},
                           ignore_index=True)
            df1.to_csv("StudentDetails.csv",index=False)
    ...
    def Fillattendances():
        ...
        if time.time() < future:
            if sub == '':
                ...
                attendance.columns=['Enrollment', 'Name', 'Date', 'Time']
                while True:
                    ret, im = cam.read()
...
def admin_panel():
    ...
    def log_in():
        ...
        if username == 'sachin6252' :
            if password == '9718091971':
                win.destroy()
                import csv
                import tkinter
                root = tkinter.Tk()
